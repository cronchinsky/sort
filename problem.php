...
if ($studentworks) {
  $mform = new sort_classify_form("/mod/sort/problem.php?id={$problem->id}", array('studentworks' => $studentworks, 'sort' => $sort));
  ...
  foreach ($files as $file) {
    ...
    foreach ($studentworks as $studentwork) {
        ...
        foreach ($categories as $key => $category) {
          $item .= "<li><a href='problem.php?id=$id&swid=$studentwork->id&cat=$key'>$category</a></li>";
        }
        $item .= '</ul>';
        $item .= '</li>';
        if (isset($studentwork->category)) {
          $category_html[$studentwork->category] .= $item;
        }
        else {
    ...
    foreach ($categories as $key => $category) {
      if ($key != 0) {
        ...
        echo "<h4 class='ui-widget-header'>$category</h4>";
        echo '<ul class="sort-gallery ui-helper-reset ui-helper-clearfix">';
        echo $category_html[$key];
        echo '</ul>';
        echo "</div>";
      }
    }
